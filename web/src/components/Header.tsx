...
export async function Header() {
  ...
  return (
    <header className="fixed left-1/2 top-8 z-50 mx-auto w-full max-w-5xl -translate-x-1/2 transform">
      <div className="border-second/50 shadow-second/10 flex items-center justify-between rounded-full border bg-black px-4 py-4 shadow-lg backdrop-blur-md">
        ...
        <div className="flex items-center space-x-4">
          {!session?.user ? (
            <Link href="/sign-in">
              <Button
                variant="ghost"
                className="bg-second text-black transition-all duration-300"
              >
                Log In
              </Button>
            </Link>
